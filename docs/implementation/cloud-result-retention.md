...
`active_retention` が空であることを確認したうえで、Firestore、GCS、Cloud Run、
Artifact Registry とローカルの EnvForge job history を横断して棚卸しした。

- Firestore `submissions` 9 件と `results` 9 件は、GCS の 9 prefix、Cloud Run の
  成功済み execution 9 件、ローカル job history 9 件と一対一で対応している。
- GCS の 9 prefix は合計 `3.17 GiB` で、各 job history から model / Replay artifact の
  再取得先として参照されているため削除しなかった。
- `job_debug` collection と soft-deleted GCS object は存在しなかった。
- Cloud Run API revision は、現行 `embodiedlab-api-00077-llp` と直前の rollback 用
  `embodiedlab-api-00076-mhr` を残した。
- トラフィックがなく、対応する Artifact Registry image も既に存在しない
  `embodiedlab-api-00074-2lj` と `embodiedlab-api-00075-wqc` を削除した。
- Notification service は現行 revision と直前 revision の 2 件だけなので変更しなかった。
- Artifact Registry の現行 image と直前 rollback image は削除しなかった。

学習結果を今後削除する場合は、先に EnvForge の job history から対象 job を整理するか、
クラウド artifact の再取得ができなくなることを明示的に受け入れてから実施する。
