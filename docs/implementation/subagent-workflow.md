...
- Phase 開始時。
- Unity、UI、ML-Agents、Sentis、docs の現状を整理したいとき。
...
- `docs/vision`
- `docs/implementation`
- `unity/EnvForge-local-first/Assets`
- `unity/EnvForge-local-first/Packages`
- 関連する Python config や training artifact
...
- 実装後。
- PR 前。
- Unity UI や replay 表示の破綻がありそうなとき。

見る観点:

- 仕様とのズレ。
- Unity scene、Prefab、ScriptableObject、package 設定の不整合。
- UI の重なり、操作不能、表示崩れ。
- ML-Agents / Sentis の入出力 shape や action mapping。
- テスト不足。
- ドキュメントと実装の食い違い。
...
EnvForge の Review Scout として、現在の差分をレビューしてください。
特に Unity UI、scene/prefab 設定、ML-Agents / Sentis 入出力、
ユーザ操作の破綻を確認してください。出力は severity 順の finding、
test gap、residual risk にしてください。ファイルは変更しないでください。

### プロンプト例: Security Scout

EnvForge の Security Scout として、現在の差分を確認してください。
特に `.env`、credential、GCS、Cloud Run、外部プロセス、ユーザ提供データの
