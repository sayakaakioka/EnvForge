...
ロボットの行動空間は、要件として連続行動のみを採用する。基本形は「0以上1以下の前進速度」と「回転量」の2次元連続値であり、停止・前進・左右旋回を連続値で表す。後退は使わない。
...
当面のMVP 2では、ロボット前方のCamera Sensorで84x84の2色画像を撮影する。通行可能領域は緑、通行不可能領域は青とする。Goalは画像には含めず、距離と角度のVector Observationで渡す。障害物、段差、未知領域はすべて通行不可能領域として青に丸める。

初期実装では画像スタックは使わず、現在フレーム1枚だけを入力する。画像観測は前方の通行可能性を担当し、既存の4値Vector ObservationはGoalまでの距離と方向を担当する。
