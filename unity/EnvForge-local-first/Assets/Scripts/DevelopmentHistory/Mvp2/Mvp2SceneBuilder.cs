...
{
    public sealed class Mvp2SceneBuilder : MonoBehaviour
    {
        ...
        private const int SegmentationImageSize = 84;
        ...
        [SerializeField] private string behaviorName = "Mvp2VisualNavigation";
        ...
        [SerializeField] private Rect segmentationPreviewRect = new(0.78f, 0.02f, 0.2f, 0.2f);

        private void Start()
        {
            Material passableMaterial = CreateUnlitMaterial("MVP 2 Passable Segmentation Material", Color.green);
            Material blockedMaterial = CreateUnlitMaterial("MVP 2 Blocked Segmentation Material", Color.blue);
            Material agentMaterial = CreateUnlitMaterial("MVP 2 Agent Material", new Color(0.16f, 0.38f, 0.78f));
            Material arrowMaterial = CreateUnlitMaterial("MVP 2 Direction Arrow Material", Color.white);
            Material goalMaterial = CreateUnlitMaterial("MVP 2 Goal Material", new Color(1f, 0.82f, 0.12f));
        ...
        private void CreateBoundaryWalls(Material material, INavigationEpisodeEvents episodeEvents)
        {
            ...
            CreateWall("MVP 2 Wall North", new Vector3(0f, wallHeight * 0.5f, halfDepth), new Vector3(floorSize.x, wallHeight, wallThickness), material, episodeEvents);
            CreateWall("MVP 2 Wall South", new Vector3(0f, wallHeight * 0.5f, -halfDepth), new Vector3(floorSize.x, wallHeight, wallThickness), material, episodeEvents);
            CreateWall("MVP 2 Wall East", new Vector3(floorSize.x * 0.5f, wallHeight * 0.5f, 0f), new Vector3(wallThickness, wallHeight, floorSize.y), material, episodeEvents);
            CreateWall("MVP 2 Wall West", new Vector3(-halfWidth, wallHeight * 0.5f, 0f), new Vector3(wallThickness, wallHeight, floorSize.y), material, episodeEvents);
        }

        private void CreateInnerWalls(Material material, INavigationEpisodeEvents episodeEvents)
        {
            CreateWall("MVP 2 Inner Wall A", new Vector3(-1.5f, wallHeight * 0.5f, -1.2f), new Vector3(0.35f, wallHeight, 4.2f), material, episodeEvents);
            CreateWall("MVP 2 Inner Wall B", new Vector3(2.8f, wallHeight * 0.5f, 1.6f), new Vector3(4.4f, wallHeight, 0.35f), material, episodeEvents);
            CreateWall("MVP 2 Inner Wall C", new Vector3(4.8f, wallHeight * 0.5f, -2.2f), new Vector3(0.35f, wallHeight, 2.5f), material, episodeEvents);
            CreateWall("MVP 2 Inner Wall D", new Vector3(-4.2f, wallHeight * 0.5f, 2.2f), new Vector3(2.4f, wallHeight, 0.35f), material, episodeEvents);
        }
        ...
        {
            GameObject arrow = new("MVP 2 Agent Direction Arrow");
            ...
            Mesh mesh = new()
            {
                name = "MVP 2 Agent Direction Arrow Mesh",
                vertices = new[]
                {
        ...
        {
            GameObject cameraObject = new("MVP 2 Segmentation Camera");
        ...
        private static void ConfigureCameraSensor(GameObject agent, Camera segmentationCamera, bool runtimeCameraEnable)
        {
            ...
            cameraSensor.Width = SegmentationImageSize;
            cameraSensor.Height = SegmentationImageSize;
        ...
        private void ConfigureSegmentationPreview(GameObject cameraObject, Camera segmentationCamera)
        {
            ...
            preview.Configure(showSegmentationPreview, segmentationCamera, segmentationPreviewRect, SegmentationImageSize, SegmentationImageSize);
        }
        ...
        private void ConfigureSegmentationCapture(GameObject cameraObject)
        {
            ...
            capture.Configure(saveSegmentationFrames, saveSegmentationEverySteps, SegmentationImageSize, SegmentationImageSize);
        }
