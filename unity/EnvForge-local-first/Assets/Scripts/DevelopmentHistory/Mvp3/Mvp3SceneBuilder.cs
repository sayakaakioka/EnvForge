...
{
    public sealed class Mvp3SceneBuilder : MonoBehaviour
    {
        ...
        [SerializeField] private string behaviorName = "Mvp3CustomCnnNavigation";
        ...
        private void Start()
        {
            Material passableMaterial = CreateUnlitMaterial("MVP 3 Passable Segmentation Material", Color.green);
            Material blockedMaterial = CreateUnlitMaterial("MVP 3 Blocked Segmentation Material", Color.blue);
            Material agentMaterial = CreateUnlitMaterial("MVP 3 Agent Material", new Color(0.16f, 0.38f, 0.78f));
            Material arrowMaterial = CreateUnlitMaterial("MVP 3 Direction Arrow Material", Color.white);
            Material goalMaterial = CreateUnlitMaterial("MVP 3 Goal Material", new Color(1f, 0.82f, 0.12f));
            ...
            Mvp3GoalObservationProvider mvp3ObservationProvider = gameObject.AddComponent<Mvp3GoalObservationProvider>();
            mvp3ObservationProvider.Configure(metrics, floorSize.magnitude);

            Mvp3NavigationAgent navigationAgent = agent.GetComponent<Mvp3NavigationAgent>();
        ...
        private void CreateBoundaryWalls(Material material, INavigationEpisodeEvents episodeEvents)
        {
            ...
            CreateWall("MVP 3 Wall North", new Vector3(0f, wallHeight * 0.5f, halfDepth), new Vector3(floorSize.x, wallHeight, wallThickness), material, episodeEvents);
            CreateWall("MVP 3 Wall South", new Vector3(0f, wallHeight * 0.5f, -halfDepth), new Vector3(floorSize.x, wallHeight, wallThickness), material, episodeEvents);
            CreateWall("MVP 3 Wall East", new Vector3(floorSize.x * 0.5f, wallHeight * 0.5f, 0f), new Vector3(wallThickness, wallHeight, floorSize.y), material, episodeEvents);
            CreateWall("MVP 3 Wall West", new Vector3(-halfWidth, wallHeight * 0.5f, 0f), new Vector3(wallThickness, wallHeight, floorSize.y), material, episodeEvents);
        }

        private void CreateInnerWalls(Material material, INavigationEpisodeEvents episodeEvents)
        {
            CreateWall("MVP 3 Inner Wall A", new Vector3(-1.5f, wallHeight * 0.5f, -1.2f), new Vector3(0.35f, wallHeight, 4.2f), material, episodeEvents);
            CreateWall("MVP 3 Inner Wall B", new Vector3(2.8f, wallHeight * 0.5f, 1.6f), new Vector3(4.4f, wallHeight, 0.35f), material, episodeEvents);
            CreateWall("MVP 3 Inner Wall C", new Vector3(4.8f, wallHeight * 0.5f, -2.2f), new Vector3(0.35f, wallHeight, 2.5f), material, episodeEvents);
            CreateWall("MVP 3 Inner Wall D", new Vector3(-4.2f, wallHeight * 0.5f, 2.2f), new Vector3(2.4f, wallHeight, 0.35f), material, episodeEvents);
        }
        ...
        private GameObject CreateAgent(Material material, Material arrowMaterial)
        {
            ...
            Mvp3NavigationAgent navigationAgent = agent.AddComponent<Mvp3NavigationAgent>();
        ...
        private void ConfigureBehaviorParameters(BehaviorParameters behaviorParameters)
        {
            ...
            behaviorParameters.BrainParameters.VectorObservationSize = Mvp3GoalObservation.ValueCount;
            behaviorParameters.BrainParameters.NumStackedVectorObservations = 1;
            behaviorParameters.BrainParameters.ActionSpec = ActionSpec.MakeContinuous(2);

            if (inferenceModel == null)
            {
                Debug.Log("MVP 3 inference model is not assigned. Using the default ML-Agents behavior.");
                return;
            }
            ...
            Debug.Log($"MVP 3 inference model assigned: {inferenceModel.name}.");
        }
        ...
        {
            GameObject arrow = new("MVP 3 Agent Direction Arrow");
            ...
            Mesh mesh = new()
            {
                name = "MVP 3 Agent Direction Arrow Mesh",
                vertices = new[]
                {
        ...
        {
            GameObject cameraObject = new("MVP 3 Segmentation Camera");
